...
function! GetNSFNameDefine()
    ...
    if idx != -1
        ...
        let define = define ."_".expand('%:t:r')."_" . ext . "_"
        let define = toupper(define)
        let define = substitute(define, '^_\+', '', '')
        return define
    else
        return toupper(expand('%:t:r'))."_" . ext . "_"
    endif
endfunction
...
XPT c hint=simple\ C++\ implementation\ file
// Copyright (c) `year()^ `$author^. All rights reserved.

`cursor^

XPT ch hint=specific\ C++\ header\ file
//
// `getNamespaceFilename()^
//
// Copyright (c) `year()^ Research In Motion
...
XPT h hint=simple\ C++\ header\ file
// Copyright (c) `year()^ `$author^. All rights reserved.
